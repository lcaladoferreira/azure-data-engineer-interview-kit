# Stream Analytics Scenario Q&A — 45
...
**Question:** A production Stream Analytics workload using **inputs** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **outputs** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **query language** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **windows** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **event ordering** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **late arrival** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **reference data** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **partitioning** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **watermarks** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **checkpointing** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **grain and data contracts** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **idempotency** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **schema evolution** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **late-arriving data** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **deduplication** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **observability** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **data quality** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **least privilege** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **encryption** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **cost optimization** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **RPO and RTO** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **backfill and replay** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **lineage** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **testing** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **deployment** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **performance tuning** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **skew** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **small files** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production Stream Analytics workload using **inputs** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production Stream Analytics workload using **outputs** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production Stream Analytics workload using **query language** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production Stream Analytics workload using **windows** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production Stream Analytics workload using **event ordering** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production Stream Analytics workload using **late arrival** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production Stream Analytics workload using **reference data** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production Stream Analytics workload using **partitioning** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production Stream Analytics workload using **watermarks** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production Stream Analytics workload using **checkpointing** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production Stream Analytics workload using **grain and data contracts** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production Stream Analytics workload using **idempotency** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production Stream Analytics workload using **schema evolution** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
