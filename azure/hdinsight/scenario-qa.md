# HDInsight Scenario Q&A — 40

40 original interview questions with concise, production-oriented answers.


## 1. Design: cluster types

**Question:** A production HDInsight workload using **cluster types** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **Hadoop** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **Spark** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **Kafka** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **HBase** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **Hive** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **security** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **storage** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **scaling** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **monitoring** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **grain and data contracts** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **idempotency** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **partitioning** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **schema evolution** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **watermarks** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **late-arriving data** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **deduplication** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **observability** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **data quality** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **least privilege** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **encryption** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **cost optimization** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **RPO and RTO** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **backfill and replay** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **lineage** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **testing** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **deployment** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **performance tuning** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **skew** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **small files** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you design first?
...
**Question:** A production HDInsight workload using **cluster types** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production HDInsight workload using **Hadoop** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production HDInsight workload using **Spark** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production HDInsight workload using **Kafka** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production HDInsight workload using **HBase** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production HDInsight workload using **Hive** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production HDInsight workload using **security** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production HDInsight workload using **storage** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production HDInsight workload using **scaling** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
...
**Question:** A production HDInsight workload using **monitoring** has rising latency, intermittent retries, and a strict recovery SLA. As the on-call data engineer, what would you troubleshoot first?
